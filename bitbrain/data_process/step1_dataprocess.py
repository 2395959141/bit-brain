...
class FormatHandler():
    ...
    def get_file_list(self) -> list:
        ...
        # 一个通用的过滤器示例，子类可能会覆盖或扩展它
        # 这个过滤器现在比较宽泛，以适应不同类型的文件扩展名
...
class IndustryCorpus2FormatHandler(FormatHandler):
    """处理IndustryCorpus2数据集（parquet格式）
    示例数据格式：
    {
        "text": "《农村财政与财务》杂志创办于1996...",
...
def main_run():
    ...
    output_path_root = "/DATA/disk2/yuhang/.cache/bit_brain_data/step1_unified_format"
    if not os.path.exists(output_path_root):
        os.makedirs(output_path_root)
