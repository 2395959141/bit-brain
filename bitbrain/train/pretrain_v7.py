...
from bitbrain.train.tools.test_model_in_pretrain import test_model_on_prompts
#! 导入SwanLab用于实验跟踪
import swanlab
...
compile_mode = "max-autotune"  # 编译模式：'default', 'reduce-overhead', 'max-autotune'
...
qwen_config.rope_theta = 10000.0            #* 原来为1000000.0
qwen_config.max_position_embeddings = 2048  #* 原来为32768
qwen_config.num_attention_heads = 16        #* 原来为14
qwen_config.num_key_value_heads = 4         #* 原来为2
qwen_config.sliding_window = 2048           #* 原来为32768
...
model = AutoModelForCausalLM.from_config(config=qwen_config,trust_remote_code=True)
...
if rank == 0:
    ...
    swanlab_config_update = {
        ...
        "weight_decay": weight_decay, #! 在SwanLab中记录权重衰减
    }
...
def train(model, optimizer, scheduler, train_loader, device,
    ...
    for batch_idx, (x, y, loss_mask) in enumerate(train_loader):
        ...
        # #! 在模型前向传播之前，如果模型已编译且在CUDA上运行，则标记CUDAGraph步骤开始
        # if is_model_compiled and torch.cuda.is_available():
