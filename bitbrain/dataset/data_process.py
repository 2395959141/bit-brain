...
def tokenize_function(examples, tokenizer, max_length, eos_token):
    ...
    try:
        ...
        for input_ids in encoding['input_ids']:
            ...
            #* 使用原始 mask，在 Dataset 的 __getitem__ 中再处理移位
...
def main():
    ...
    parser.add_argument("--num_proc", type=int, default=20, help="Number of processes to use for map function. Defaults to cpu count.")
    parser.add_argument("--file_ratio", type=float, default=0.4, help="Ratio of data to process. Defaults to 0.4.")
    ...
    for data_path in args.data_paths:
        ...
        #! 使用多个模式匹配不同扩展名
        patterns = [
            os.path.join(data_path, '**', '*.parquet'), #* 递归目录匹配
            os.path.join(data_path, '**', '*.json'),
            os.path.join(data_path, '**', '*.jsonl'),
            os.path.join(data_path, '*.parquet'),  #* 当前目录匹配
            os.path.join(data_path, '*.json'),
            os.path.join(data_path, '*.jsonl')
        ]
        ...
        #! 文件级别去重
        # todo: 由于pattern写法会重复读取，所以这里需要去重
