...
OUTPUT_DIR=/DATA/disk2/yuhang/.cache/ckpt/bit-brain-v3/sft
...
FORCE_TORCHRUN=1 CUDA_VISIBLE_DEVICES=4,5,6,7  torchrun --nproc_per_node 4 $LLaMA_PATH/src/train.py \
    ...
    --dataset shared_gpt_format,deepctl_1120W_zh,baai_instruct_70W,ultrachat_200k,baai_instruct_682W,code_feedback_custom \
    --max_samples 1000000 \
    --overwrite_cache \
    --packing False \
    --use_swanlab true \
    --report_to swanlab \
    --swanlab_project bit-brain-v3-sft \
    --run_name sft_bit-brain-v3 \
    ...
    --plot_loss \
    --bf16 \
    #--resume_from_checkpoint /DATA/disk2/yuhang/.cache/ckpt/bit-brain-v2/sft/sft_mix2/checkpoint-10000
